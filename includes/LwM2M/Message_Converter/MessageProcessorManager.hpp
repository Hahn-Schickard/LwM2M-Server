...
template <typename T> class MessageProcessorRunner : public Stoppable {
  std::unique_ptr<Converter<T>> converter_;
  std::shared_ptr<ThreadsafeQueue<T>> message_buffer_;
  std::shared_ptr<HaSLL::Logger> logger_;

public:
  MessageProcessorRunner() {}

  MessageProcessorRunner(std::unique_ptr<Converter<T>> converter,
...
template <typename T> struct MessageProcessor {
  std::thread thread_id_;
  MessageProcessorRunner<T> processor_;
...
class MessageProcessorManager {
  ...
  MessageProcessor<CoAP::CoAP_Message> incoming_message_processing_thread_;
  MessageProcessor<LwM2M_Message> outgoing_message_processing_thread_;

public:
  MessageProcessorManager(std::shared_ptr<ThreadsafeQueue<CoAP::CoAP_Message>>
