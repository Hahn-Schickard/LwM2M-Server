...
CoAP_Server::CoAP_Server(DeviceRegistryPtr registry, const CoAP_Config &config)
    : Registrator(registry), Socket(config.address_, config.port_),
...
void CoAP_Server::start() {
  Socket::start();
  logger_->log(SeverityLevel::INFO, "CoAP Server started!");
}

void CoAP_Server::stop() {
  Socket::stop();
  logger_->log(SeverityLevel::INFO, "CoAP Server stoped!");
}

ClientResponsePtr CoAP_Server::makeClientResponse(CoAP::MessagePtr message) {
  auto endpoint =
...
future<DataFormatPtr> CoAP_Server::requestData(ServerRequestPtr message) {
...
CoAP_Server::requestMultiTargetData(ServerRequestPtr message) {
  return async(launch::async,
...
future<bool> CoAP_Server::requestAction(ServerRequestPtr message) {
  return async(launch::async,
...
void CoAP_Server::handleNotification(CoAP::MessagePtr message) {
  logger_->log(SeverityLevel::CRITICAL,
...
ServerResponsePtr
CoAP_Server::handleRegistrationRequest(CoAP::MessagePtr message) {
  auto options = message->getOptions();
...
ServerResponsePtr CoAP_Server::handleRequest(CoAP::MessagePtr message) {
  logger_->log(SeverityLevel::TRACE,
...
void CoAP_Server::handleReceived(CoAP::MessagePtr message) {
