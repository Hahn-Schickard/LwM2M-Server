...
makeResourcePtr(shared_ptr<Endpoint> endpoint, uint32_t parent_id,
                uint32_t instance_id, shared_ptr<MessageEncoder> encoder,
                ResourceDescriptorPtr descriptor) {
  switch (descriptor->operations_) {
  case OperationsType::READ: {
    return make_shared<Readable<T>>(endpoint, parent_id, instance_id, encoder,
                                    descriptor);
  }
  case OperationsType::WRITE: {
    return make_shared<Writable<T>>(endpoint, parent_id, instance_id, encoder,
                                    descriptor);
  }
  ...
  case OperationsType::EXECUTE: {
    return make_shared<Executable<T>>(endpoint, parent_id, instance_id, encoder,
                                      descriptor);
  }
  case OperationsType::NO_OPERATION:
  default: {
    return make_shared<Resource<T>>(endpoint, parent_id, instance_id, encoder,
                                    descriptor);
  }
  }
}
