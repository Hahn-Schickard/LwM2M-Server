...
namespace utility {
vector<uint8_t> toBytes(CoAP::ObjectLink value) {
  throw getHeader()->runtime_error(
...
TLV_Pack::TLV_Pack(vector<uint8_t> bytestream) : ContentFormatType(11542) {
  do {
...
TLV_Pack::TLV_Pack(TLV_ValueMap values)
    : ContentFormatType(11542), values_(values) {}
...
template <>
CoAP::CoRE_Link TLV_Pack::getValue<CoAP::CoRE_Link>(uint16_t identifier) {
  string value = getValue<string>(identifier);
  auto link_value = CoAP::CoRE_Link(value);
  return link_value;
}
...
template <> TLV_Pack decode(PayloadPtr payload) {
  if (payload->getContentFormat()->getIndex() != 11542) {
    throw WrongContentFormatTypeDecoder("TLV", payload->getContentFormat());
  }
  return TLV_Pack(payload->getBytes());
}

template <> PayloadPtr encode(TLV_Pack content) {
  return make_shared<Payload>(content.getContentFormat()->getIndex(),
                              content.getBytes());
