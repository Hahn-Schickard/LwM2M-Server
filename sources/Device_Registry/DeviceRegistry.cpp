...
void DeviceRegistry::logListenerException(const exception_ptr& eptr) {
  try {
    ...
    logger_->error("One of the listeners threw an exception, while handing an "
                   "event notification. Exception: {}",
        ex.what());
  } catch (...) {
    logger_->critical("Caught an unkown unhandled exception.");
...
void DeviceRegistry::registerDevice(DevicePtr new_device) {
  if (isRegistered(new_device->getDeviceId())) {
    logger_->warning("Device {} with id {} already exists in the registry!",
